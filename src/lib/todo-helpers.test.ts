...
describe("todo helpers", () => {
  ...
  it("builds correct section metadata", () => {
    ...
    expect(getSectionMeta(todo, "priority", now)).toEqual({
      key: "high",
      label: "High priority",
      order: 0,
    });
    expect(getSectionMeta(todo, "status", now)).toEqual({
      key: "active",
      label: "Active",
      order: 0,
    });
    expect(getSectionMeta(todo, "deadline", now)).toEqual({
      key: "today",
      label: "Due today",
      order: -1,
    });
    expect(getSectionMeta(todo, "group", now)).toEqual({
      key: "ops",
      label: "Ops",
      order: "o".charCodeAt(0),
    });
  });
  ...
  it("builds deadline metadata for no-deadline and overdue states", () => {
    ...
    expect(getSectionMeta(createTodo(), "deadline", now)).toEqual({
      key: "no-deadline",
      label: "No deadline",
      order: Number.MAX_SAFE_INTEGER,
    });
    expect(getSectionMeta(createTodo({ deadline: "2026-03-23" }), "deadline", now)).toEqual({
      key: "overdue",
      label: "Overdue",
      order: -2,
    });
    expect(getSectionMeta(createTodo(), "none", now)).toEqual({
      key: "all",
      label: "All todos",
      order: 0,
    });
  ...
  it("groups no-grouping mode into a single section", () => {
    const sections = groupTodos(
      [createTodo({ id: "1", title: "One" }), createTodo({ id: "2", title: "Two" })],
      "none",
    );
