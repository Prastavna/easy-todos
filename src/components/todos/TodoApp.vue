...
function saveTodo() {
  ...
  if (editingId.value) {
    todos.value = todos.value.map((todo) =>
      todo.id === editingId.value ? { ...todo, ...payload, updatedAt: now } : todo,
    );
  } else {
    todos.value.unshift({
...
const dueTodayCount = computed(
  () => todos.value.filter((todo) => isDueToday(todo) && !todo.completed).length,
);
...
const expandedSections = computed({
  get: () =>
    groupedTodos.value
      .filter(
        (section) => !collapsedSections.value[getSectionStorageKey(groupBy.value, section.key)],
      )
...
const containerClass = computed(() =>
  isPopup.value
    ? "mx-auto flex min-h-screen w-full max-w-[440px] flex-col gap-2 px-3 py-2 pt-3 pb-14"
    ...
    <div :class="containerClass">
      ...
      <div class="space-y-2">
        ...
        <main class="space-y-2">
          ...
          <Card
            v-else
            class="border-dashed border-slate-300 bg-white/85 py-8 shadow-[0_18px_70px_-45px_rgba(15,23,42,0.5)] backdrop-blur"
          >
            <CardContent class="flex flex-col items-center justify-center gap-4 text-center">
              ...
              <div class="flex flex-col gap-2 sm:flex-row">
                <Button class="rounded-xl" @click="openCreateDialog"> Create todo </Button>
